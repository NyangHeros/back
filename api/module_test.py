...
def cat_snn(img_path):
    ...
    # 2. 고양이 이름들 : 임의 지정.
    names = ["nabi", "nana", "lulu", "hi", "cati",
            "flower", "bella", "simba", "luna", "roky"]
    ...
    # └── 0152
    #     └── 0152_008.JPG
    archer_dir = r"api/archer_img/"

    # 9. 결과 예측
    # 9-1. 가장 유사한 고양이의 Id 받기 : predictId 함수 사용.
    #      최소값이 dist_threshold 보다 커서 유사하지 않으면 unknown으로 판정된다.
    #      출력 값이 -1이면 unknown 으로 판정된 것이다.
    res = catSnn.predictId(selection=selections, image_path=img_path,
                        archer_dir=archer_dir,  dist_threshold=0.4)
